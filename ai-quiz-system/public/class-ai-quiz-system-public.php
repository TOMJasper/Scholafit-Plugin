...
class AI_Quiz_System_Public {
    ...
    public function api_start_quiz($request) {
        ...
        $max_questions = $exam->question_count_per_subject * count($subject_ids);
    ...
    public function api_submit_answer($request) {
        ...
        // Verify question ID matches (if question has ID from database)
    ...
    public function get_exam_subjects() {
        ...
        if (empty($subjects)) {
            wp_send_json_error(array('message' => __('No subjects found for this exam.', 'ai-quiz-system')));
        }
